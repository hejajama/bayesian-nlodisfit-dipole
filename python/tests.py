...
class TestNlodipole(unittest.TestCase):
    def test_GBW(self):
        ...
        Y = 0.5
        gbwinterpolator = nlodipole.ReadBKDipole(bk_file_dir)

        r_values = [1e-2, 1e-1, 1, 10]  # Example r values
        N_values = [gbwinterpolator(Y, r) for r in r_values]
        ...
        r=0.2
        Y=2.6
        model=nlodipole.get_NRY(bk_file_dir, Y=Y, r=r)
    ...
    def test_bal_sd_qs(self):
        ...
        true_qs2=0.196
        dipval = interpolator(Y, np.sqrt(2/true_qs2))
        self.assertAlmostEqual(dipval, 1 - np.exp(-0.5), places=2)

        bk_file_median = "../data/balsd/bk_median.dat"
        interpolator_median = nlodipole.ReadBKDipole(bk_file_median)
    ...
    def test_parent_qs(self):
        ...
        interpolator_median = nlodipole.ReadBKDipole(bk_file_dir)
